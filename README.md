...
- **The parent never blocks on MiniLM.** Query embedding runs on the interactive first turn only when the vector index is non-empty; an unavailable embedder degrades to lexical-only retrieval.

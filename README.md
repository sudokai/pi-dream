...
| Surface            | Behavior                                                                                                                                                                                                         |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First turn         | Synthesizer turns the top layer of the memory tree into a grounded answer + a categorized index of the remaining roots (preferences, facts, summaries; heat-ordered; visible `pi-dream-briefing` custom message) |
...
| `briefingTokenBudget`      | 8000                      | Estimated-token cap for the top layer (consolidation-enforced; reads fail closed above it, and an over-budget layer waives the cadence gates so the next settle runs urgent consolidation). Must be ≥ 200 (the largest single-node estimate). |
